 ...
 */
function registerPreviewTools(server, apiClient, authManager, logger) {
    // Create preview tool
 ...
 */
function registerSearchTools(server, apiClient, authManager, logger) {
    // Search previews tool
    server.tool("searchPreviews", {
        query: zod_1.z.string().describe("Search terms to filter previews by name or other properties"),
        state: zod_1.z.enum(['all', 'ready', 'building', 'failed']).optional().describe("Filter by preview state")
    }, async ({ query, state }) => {
        try {
            // Check authorization
            ...
            try {
                ...
                if (matchingPreviews.length === 0) {
                    ...
                    matchingPreviews.forEach((preview, index) => {
                        ...
                        if (preview.url)
                            resultText += `   URL: ${preview.url}\n`;
                        if (preview.created)
                            resultText += `   Created: ${new Date(preview.created).toLocaleString()}\n`;
                        resultText += '\n';
                    });
                }
                return {
    ...
    server.tool("searchProjects", {
        query: zod_1.z.string().describe("Search terms to filter projects by name or other properties")
    }, async ({ query }) => {
        try {
            // Check authorization
 ...
 */
function registerProjectTools(server, apiClient, authManager, logger) {
    // List projects tool
    server.tool("listProjects", "List all projects that the authenticated user has access to", async (extra) => {
        try {
            // Check authorization
            const isAuthorized = await authManager.isAuthorized('projects', 'read');
            if (!isAuthorized) {
    ...
    server.tool("updateProject", {
        ...
        domain: zod_1.z.string().optional().describe("Default domain for Preview links")
    }, async ({ id, name, domain }, extra) => {
        try {
            // Check authorization
    ...
    server.tool("deleteProject", {
        id: zod_1.z.string()
            ...
            .max(24, "Project ID must be at most 24 characters")
            .describe("Project ID (24 characters)"),
        confirm: zod_1.z.boolean().describe("Confirmation to delete the project")
    ...
    server.tool("getProjectJobs", {
        id: zod_1.z.string()
            .min(24, "Project ID must be at least 24 characters")
            .max(24, "Project ID must be at most 24 characters")
            .describe("Project ID (24 characters)"),
        children: zod_1.z.boolean().optional().describe("Include jobs for repos, previews, and services"),
        limit: zod_1.z.number().optional().describe("Number of jobs to return")
    }, async ({ id, children, limit }, extra) => {
        try {
            // Check authorization
            const isAuthorized = await authManager.isAuthorized('projects', 'read');
            if (!isAuthorized) {
            ...
            if (children !== undefined)
                queryParams.push(`children=${children}`);
            if (limit !== undefined)
    ...
    server.tool("getProjectStats", {
        id: zod_1.z.string()
            .min(24, "Project ID must be at least 24 characters")
            .max(24, "Project ID must be at most 24 characters")
            .describe("Project ID (24 characters)"),
        item: zod_1.z.enum(['build-time', 'refresh-time', 'size', 'repos', 'previews', 'services'])
            .describe("Type of statistics to retrieve"),
        after: zod_1.z.string().optional().describe("Only return results after this date/time"),
        before: zod_1.z.string().optional().describe("Only return results before this date/time"),
        limit: zod_1.z.number().optional().describe("Number of data points to return")
    }, async ({ id, item, after, before, limit }, extra) => {
        try {
            // Check authorization
            const isAuthorized = await authManager.isAuthorized('projects', 'read');
            if (!isAuthorized) {
            ...
            stats.forEach((stat, index) => {
                ...
                if (stat.preview)
                    resultText += `   Preview: ${stat.preview}\n`;
                if (stat.repo)
                    resultText += `   Repo: ${stat.repo}\n`;
                if (stat.service)
                    resultText += `   Service: ${stat.service}\n`;
                resultText += '\n';
            });
            return {
                content: [
 ...
 */
function registerRepositoryTools(server, apiClient, authManager, logger) {
    ...
    server.tool("createRepository", {
        ...
        provider: zod_1.z.object({
            name: zod_1.z.enum(["bitbucket", "git", "github", "gitlab", "stash"])
                .describe("Git provider name")
        }).describe("Information about the provider hosting the repository"),
        repository: zod_1.z.object({
            group: zod_1.z.string().describe("Repository owner, organization or group"),
            name: zod_1.z.string().describe("Repository name")
        }).describe("Information about the provider repository"),
        auth: zod_1.z.object({
            token: zod_1.z.string().optional().describe("Personal access token"),
            access: zod_1.z.string().optional().describe("OAuth access token"),
            user: zod_1.z.string().optional().describe("Username"),
            pass: zod_1.z.string().optional().describe("Password")
        }).optional().describe("Provider authentication information"),
        name: zod_1.z.string().optional().describe("Human-readable label for the Repository"),
        autobuild: zod_1.z.boolean().optional().describe("Whether to automatically create a Preview when a pull request is created"),
        autobuild_drafts: zod_1.z.boolean().optional().describe("Whether to automatically create or update draft pull requests"),
        autodelete: zod_1.z.boolean().optional().describe("Whether to automatically delete a Preview when its pull request is merged or closed"),
        autorebuild: zod_1.z.boolean().optional().describe("Whether to automatically rebuild a Preview when code is pushed"),
        autoredeploy: zod_1.z.boolean().optional().describe("When new code is pushed, deploy the new code without losing non-code changes"),
        domain: zod_1.z.string().optional().describe("Default domain for Preview links"),
        provider_comment: zod_1.z.boolean().optional().describe("Whether to add a comment to the pull request after build/rebuild/refresh"),
        provider_deployment: zod_1.z.boolean().optional().describe("Whether to update the provider's deployment API"),
        provider_forks: zod_1.z.boolean().optional().describe("Whether Previews are allowed from forked repositories"),
        provider_status: zod_1.z.boolean().optional().describe("Whether to update the provider's status API"),
        build_timeout: zod_1.z.number().min(1).max(18000).optional().describe("How long to let a Preview build run before timeout (seconds)")
    }, async ({ project, provider, repository, auth, name, ...options }, extra) => {
        try {
            // Check authorization
    ...
    server.tool("deleteRepository", {
        id: zod_1.z.string()
            ...
            .max(24, "Repository ID must be at most 24 characters")
            .describe("Repository ID"),
        confirm: zod_1.z.boolean().describe("Confirmation to delete the repository")
    }, async ({ id, confirm }, extra) => {
        try {
            // Check authorization
    ...
    server.tool("updateRepositoryAuth", {
        id: zod_1.z.string()
            .min(24, "Repository ID must be at least 24 characters")
            .max(24, "Repository ID must be at most 24 characters")
            .describe("Repository ID"),
    ...
    server.tool("getRepositoryPreviews", {
        id: zod_1.z.string()
            .min(24, "Repository ID must be at least 24 characters")
            .max(24, "Repository ID must be at most 24 characters")
            .describe("Repository ID")
    ...
    server.tool("getRepositoryBranches", {
        id: zod_1.z.string()
            .min(24, "Repository ID must be at least 24 characters")
            .max(24, "Repository ID must be at most 24 characters")
            .describe("Repository ID")
    ...
    server.tool("getRepositoryPullRequests", {
        id: zod_1.z.string()
            .min(24, "Repository ID must be at least 24 characters")
            .max(24, "Repository ID must be at most 24 characters")
            .describe("Repository ID")
    ...
    server.tool("getRepositoryJobs", {
        id: zod_1.z.string()
            .min(24, "Repository ID must be at least 24 characters")
            .max(24, "Repository ID must be at most 24 characters")
            .describe("Repository ID"),
        action: zod_1.z.array(zod_1.z.enum(["keygen", "update"])).optional().describe("Filter jobs by action"),
        children: zod_1.z.boolean().optional().describe("Include jobs for previews and services"),
        limit: zod_1.z.number().optional().describe("Number of jobs to return")
    }, async ({ id, action, children, limit }, extra) => {
        try {
            // Check authorization
            const isAuthorized = await authManager.isAuthorized('repos', 'read');
            if (!isAuthorized) {
    ...
    server.tool("getRepositoryRegistries", {
        id: zod_1.z.string()
            .min(24, "Repository ID must be at least 24 characters")
            .max(24, "Repository ID must be at most 24 characters")
            .describe("Repository ID")
    ...
    server.tool("getRepositoryStats", {
        id: zod_1.z.string()
            .min(24, "Repository ID must be at least 24 characters")
            .max(24, "Repository ID must be at most 24 characters")
            .describe("Repository ID"),
        item: zod_1.z.enum(['build-time', 'refresh-time', 'size', 'previews', 'services'])
            .describe("Type of statistics to retrieve"),
        after: zod_1.z.string().optional().describe("Only return results after this date/time"),
        before: zod_1.z.string().optional().describe("Only return results before this date/time"),
        limit: zod_1.z.number().optional().describe("Number of data points to return")
    ...
    server.tool("createRepositorySSHKey", {
        id: zod_1.z.string()
            .min(24, "Repository ID must be at least 24 characters")
            .max(24, "Repository ID must be at most 24 characters")
            .describe("Repository ID"),
        type: zod_1.z.enum(["rsa", "dsa", "ecdsa", "ed25519"]).optional().describe("SSH key type"),
        bits: zod_1.z.number().optional().describe("SSH key bit length")
    }, async ({ id, type, bits }, extra) => {
        try {
            // Check authorization
            const isAuthorized = await authManager.isAuthorized('repos', 'write');
            if (!isAuthorized) {
